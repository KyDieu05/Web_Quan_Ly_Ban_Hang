...
namespace QuanLyNhaHang.Areas.NhanVienBep.Controllers
{
    public class HoaDonController : Controller
    {
        ...
        public ActionResult ThongTinHoaDon(int iMaBan)
        {
            ...
            if (ban != null) // nếu bàn trống
            {
                ...
                db.HoaDons.Add(hd);
                db.SaveChanges();
                var hoaDonTrong = db.HoaDons.SingleOrDefault(n => n.MaBan_id == iMaBan && n.TrangThai != 0); //bàn đã có hóa đơn chưa thanh toán 
            ...
            {
                var hoaDon = db.HoaDons.SingleOrDefault(n => n.MaBan_id == iMaBan && n.TrangThai != 0); //bàn đã có hóa đơn chưa thanh toán 
        ...
        public ActionResult CapNhatSoLuong(int iMaHoaDon, int iMaMonAn, string strURL, FormCollection f)
        {
            ...
            var monAn = db.ChiTietHoaDons.Include("MonAn").SingleOrDefault(n => n.MaHoaDon_id == iMaHoaDon && n.MaMonAn_id == iMaMonAn);
            // Lấy giá món ăn
            MonAn giaMonAn = db.MonAns.SingleOrDefault(n => n.MaMonAn == iMaMonAn);
            if (soLuong >= 0)
            {
        ...
        public ActionResult ThanhToan(int iMaHoaDon, FormCollection f)
        {
            ...
            {
                return RedirectToAction("DaThanhToan", "Error");
            }
            else
            {
                return RedirectToAction("KhongThanhCong", "Error");
            }
        }
        ...
        // danh sách hóa đơn
        #region Hiển thị danh sách hóa đơn theo trạng thái

        // Hóa đơn mới tạo (TrangThai == 1)
        public ActionResult DSHoaDonMoiTao()
        {
            var list = db.HoaDons
        ...
        // Hóa đơn đã phục vụ (TrangThai == 2)
        public ActionResult DSHoaDonDaPhucVu()
        {
            var list = db.HoaDons
        ...
        // Hóa đơn đã thanh toán (TrangThai == 0)
        public ActionResult DSHoaDonDaThanhToan()
        {
            var list = db.HoaDons
        ...
        public ActionResult DieuHuong(int iMaHoaDon)
        {
            var hoaDon = db.HoaDons.SingleOrDefault(n => n.MaHoaDon == iMaHoaDon);
            if (hoaDon == null)
            {
            ...
            // Nếu hóa đơn mới tạo (1), chuyển sang đã phục vụ (2)
            if (hoaDon.TrangThai != 0)
            {
                hoaDon.TrangThai = 2;
                db.SaveChanges();
            }
            // Điều hướng theo trạng thái hiện tại
            return RedirectToAction("DanhSachHoaDon", "HoaDon");
        }
        // Xác nhận thành lập hóa đơn:
        public ActionResult XacNhan(int iMaHoaDon)
        {
            var hoaDon = db.HoaDons.SingleOrDefault(n => n.MaHoaDon == iMaHoaDon);
            if (hoaDon != null)
            {
