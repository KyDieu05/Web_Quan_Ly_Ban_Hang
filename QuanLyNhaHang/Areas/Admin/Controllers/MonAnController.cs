...
namespace QuanLyNhaHang.Areas.Admin.Controllers
{
    public class MonAnController : Controller
    {
        ...
        [HttpGet]
        public ActionResult CapNhatMonAn(int iMaMonAn)
        {
            var monAn = db.MonAns.Find(iMaMonAn);
            if (monAn == null)
            {
            ...
            // Lấy chi tiết nguyên liệu đã dùng cho món ăn
            ViewBag.ChiTietSanPham = db.ChiTietSanPhams.Where(n => n.MaMonAn_id == iMaMonAn).ToList();
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult CapNhatMonAn(HttpPostedFileBase HinhAnh, MonAn Model, IEnumerable<ChiTietSanPham> listCTSP)
        {
            try
            {
                var monAn = db.MonAns.Find(Model.MaMonAn);
                if (monAn == null)
                {
                    return HttpNotFound();
                }
                ...
                #region Cập nhật hình ảnh nếu có
                if (HinhAnh != null && HinhAnh.ContentLength > 0)
                {
                    ...
                    monAn.HinhAnh = tenAnh;
                }
                #endregion

                #region Cập nhật Chi tiết sản phẩm
                // Xoá chi tiết cũ
                var oldCT = db.ChiTietSanPhams.Where(n => n.MaMonAn_id == monAn.MaMonAn).ToList();
                db.ChiTietSanPhams.RemoveRange(oldCT);
                ...
                if (listCTSP != null)
                {
                    foreach (var item in listCTSP)
                    {
                        ChiTietSanPham ctsp = new ChiTietSanPham
                        {
                            MaMonAn_id = monAn.MaMonAn,
                            MaNguyenLieu_id = item.MaNguyenLieu_id,
